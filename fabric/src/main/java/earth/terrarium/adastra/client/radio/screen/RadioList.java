...
public class RadioList extends AbstractWidget {
    ...
    public class RadioEntry {
        ...
        protected void render(@NotNull GuiGraphicsExtractor graphics, int id, int left, int top, int width, int height, int mouseX, int mouseY, boolean hovered, float partialTick, boolean selected) {
            ...
            if ((favorite || hovered) && this.info != null) {
                ...
                graphics.text(Minecraft.getInstance().font, text, textStart, top + 3, favorite ? 0xFFFFAA00 : 0xFFFFFFFF);
                textStart += Minecraft.getInstance().font.width(text) + 2;
            }
            graphics.text(Minecraft.getInstance().font, getName(), textStart, top + 3, 0xFFFFFFFF);
