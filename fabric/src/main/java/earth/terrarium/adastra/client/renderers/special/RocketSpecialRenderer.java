...
public class RocketSpecialRenderer implements NoDataSpecialModelRenderer {
    ...
    @Override
    public void submit(PoseStack poseStack, SubmitNodeCollector collector, int light, int overlay, boolean foil, int color) {
        poseStack.pushPose();
        // TODO 26.1.2: NoDataSpecialModelRenderer.submit no longer receives ItemDisplayContext;
        // apply a sensible default transform until the bake context exposes the context.
        applyContextTransform(ItemDisplayContext.NONE, poseStack);
    ...
    public record Unbaked(Identifier texture, String layer) implements SpecialModelRenderer.Unbaked<Void> {
