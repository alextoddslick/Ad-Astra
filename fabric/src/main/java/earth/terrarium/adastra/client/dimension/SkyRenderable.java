...
public record SkyRenderable(
    ...
    public static final Codec<SkyRenderable> CODEC = RecordCodecBuilder.create(inst -> inst.group(
        Identifier.CODEC.fieldOf("texture").forGetter(SkyRenderable::texture),
    ...
    public SkyRenderable(Identifier texture, float scale, Vec3 globalRotation, Vec3 localRotation, MovementType movementType, int backLightColor) {
        this(texture, scale, globalRotation, localRotation, movementType, false, backLightColor, scale * 3);
    }

    public SkyRenderable(Identifier texture, float scale, Vec3 globalRotation, Vec3 localRotation, MovementType movementType, boolean blend, int backLightColor) {
