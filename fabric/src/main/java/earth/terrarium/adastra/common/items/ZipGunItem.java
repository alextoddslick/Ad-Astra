...
public class ZipGunItem extends Item {
    ...
    public void onUseTick(@NotNull Level level, @NotNull LivingEntity entity, @NotNull ItemStack stack, int remainingUseDuration) {
        ...
        if (level.getRandom().nextInt(particleChance) == 0) {
            level.addParticle(
                ParticleTypes.SNOWFLAKE,
                entity.getX(),
                entity.getY() + 1.0,
                entity.getZ(),
                lookAngle.x * particleSpeed + level.getRandom().nextGaussian() * 0.03,
                lookAngle.y * particleSpeed + level.getRandom().nextGaussian() * 0.03,
                lookAngle.z * particleSpeed + level.getRandom().nextGaussian() * 0.03
            );
        }
    }
