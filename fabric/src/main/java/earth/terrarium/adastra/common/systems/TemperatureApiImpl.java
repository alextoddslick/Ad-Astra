...
public class TemperatureApiImpl implements TemperatureApi {
    ...
    public void entityTick(ServerLevel level, LivingEntity entity) {
        ...
        if (entity.getType().builtInRegistryHolder().is(ModEntityTypeTags.CAN_SURVIVE_IN_SPACE)) return;
        if (SpaceSuitItem.hasFullSet(entity, ModItemTags.SPACE_RESISTANT_ARMOR)) return;
        if (this.isHot(level, entity.blockPosition())) {
            if (entity.getType().builtInRegistryHolder().is(ModEntityTypeTags.CAN_SURVIVE_EXTREME_HEAT)) return;
            ...
            if (entity.getType().builtInRegistryHolder().is(ModEntityTypeTags.CAN_SURVIVE_EXTREME_COLD)) return;
    ...
    private void freezeEntity(LivingEntity entity, ServerLevel level) {
        ...
        if (AdAstraEvents.ColdTemperatureTickEvent.fire(level, entity)) {
            ...
            ModUtils.sendParticles(level,
                ...
                entity.getY() + 1,
                entity.getZ(), 1,
                Mth.randomBetween(level.getRandom(), -1.0f, 1.0f) * 0.085f,
                0.05,
                Mth.randomBetween(level.getRandom(),
                    -1.0f,
                    1.0f) * 0.085,
                0);
        }
