...
- **2026-04-30 follow-up #3 (depth-occlusion fix)**: User reported tiny stars + sun bleeding through the Earth disc in earth_orbit. Root cause: `RenderPipelines.CELESTIAL` is `BlendFunction.OVERLAY` + `withDepthWrite(false)` — purpose-built for tiny additive sun/moon sprites, never opaque planet bodies, so anything drawn before *or* after a disc at the same screen pixel blends through. Fix in `PlanetSkyRenderer`: added new private `RenderPipeline PLANET_DISC_PIPELINE` mirroring `CELESTIAL` (POSITION_TEX + `core/position_tex` shaders, same uniforms via the existing `RenderSystem.bindDefaultUniforms` call) but with `BlendFunction.TRANSLUCENT` (soft texture-alpha edges) + `LEQUAL_DEPTH_TEST` + `withDepthWrite(true)`. Body discs now render on this pipeline; back-light glows still render on `CELESTIAL` (additive halo around the disc, drawn first / behind). The custom `ad_astra_sky` framepass already binds the main render target's depth view directly via `createCommandEncoder().createRenderPass(... depth, OptionalDouble.empty())` so depth writes work without modifying `LevelRendererSkyPassMixin` or the FrameGraph wiring — vanilla `addMainPass` runs after sky and rebinds + uses depth as normal, so this doesn't break terrain occlusion. Both fabric + neoforge `./gradlew build` SUCCESSFUL.
  - Pipeline location id `adastra:pipeline/planet_disc`. Built directly via `RenderPipeline.builder().build()` (no `RenderPipelines.register` — that map is private and only used for resource-pack overrides).
  - **Visual risk left:** between two Ad Astra discs at the same `y=100` radius but different rotations (e.g. sun and Earth in `earth_orbit`), if both project to overlapping depth values at the same screen pixel, LEQUAL allows whichever draws second to overpaint — but in practice their angular separation puts them at different post-projection depths, and the `skyRenderables` list defines a stable draw order. If overlap shows up in testing, fall back to a back-to-front sort by `scale` descending (largest disc last) on the renderables list.
- **2026-04-30 follow-up #2 (kill End purple + End sounds)**: User reported still seeing the End purple void texture and hearing End-style ambient drones in moon / mercury / orbit dims. Fix:
  - Switched **all** Ad Astra space `dimension_type` JSONs (orbit + airless surface) from `"skybox": "end"` to `"skybox": "none"` so vanilla never draws the End sky pass at all (no `end_sky.png` purple void, no End-tied audio path). Atmospheric surface dims (mars / venus / glacio) were already on `none` — colour `attributes` retained for atmospheric tint.
  - Added new `LevelRendererSkyPassMixin` (`@Inject` HEAD on `LevelRenderer.addSkyPass`, cancellable). When `ClientPlatformUtils.getPlanetRenderers()` has an entry for the current dim, we add our own `FrameGraphBuilder` pass (named `ad_astra_sky`) that reuses vanilla's `SkyRenderer.renderStars` invoker for the star field, then calls `PlanetSkyRenderer.render(...)` for the planet/sun/moon discs, and `ci.cancel()` the original method so its NONE-early-return never runs. For non-Ad-Astra dims the mixin is a no-op (early-return before cancel) — overworld / nether / vanilla End / modded skies are untouched.
  - Added `SkyRendererAccessor` interface mixin exposing private `renderStars(float, PoseStack)` so we can reuse vanilla's 1500-star buffer rather than re-implementing it. Star brightness pinned to 1.0 (full) — we are in space, no atmospheric dimming.
  - Added `getSkyRenderer` / `getTargets` accessors on `LevelRendererAccessor` so the new mixin can read the private `LevelRenderer.skyRenderer` and the `LevelTargetBundle` it needs to bind the framepass to `targets.main`.
  - Removed old `SkyRendererMixin` (its TAIL injections targeted methods that vanilla never calls when `skybox == NONE`, so they were dead code under the new scheme). Removed from mixins config.
  - Removed `mood_sound: minecraft:ambient.cave` from all 10 Ad Astra space biome JSONs (`worldgen/biome/{orbit,lunar_wastelands,mercury_deltas,martian_*,venus_*,glacio_*,infernal_venus_barrens}.json`). The cave drone doubles as the End drone the user was hearing — biomes now have no ambient mood sound. (Music tracks not touched.)
  - Both fabric + neoforge `./gradlew build` SUCCESSFUL.
  - **In-game verification needed:** load any orbit dim or moon/mercury/mars/venus/glacio surface; confirm (a) no End purple void texture, (b) no cave/portal drone, (c) star field draws, (d) planet discs draw on top of stars. If discs flicker / star field is too dim, suspect the `RenderSystem.setShaderFog(shaderFog)` call ordering or that we need to pass the actual `starBrightness` from the dim renderer rather than 1.0.
