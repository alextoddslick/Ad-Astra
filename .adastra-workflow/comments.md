...
- 2026-04-30: User reported stars + sun visible **through** the Earth disc — Earth was supposed to be opaque.
  > AI 2026-04-30: ✓ **fixed.** `RenderPipelines.CELESTIAL` is OVERLAY-blend + no-depth-write — designed for tiny additive sun/moon sprites, not opaque planet bodies, so anything drawn before *or* after a disc at the same screen pixel blends through. Built a custom `PLANET_DISC_PIPELINE` in `PlanetSkyRenderer` that mirrors CELESTIAL's POSITION_TEX + `core/position_tex` shaders but with `BlendFunction.TRANSLUCENT` + `LEQUAL_DEPTH_TEST` + `withDepthWrite(true)`. Body discs now render opaque & write depth → stars (drawn first, no depth write) get depth-tested out behind Earth, and any later-drawn celestial body gets occluded too. Backlight glow stays on `CELESTIAL` (additive halo around the disc, drawn first / behind, exactly as intended). Custom `ad_astra_sky` framepass already binds the main render target's depth view directly via `createCommandEncoder().createRenderPass(... depth, OptionalDouble.empty())` so depth writes work without touching `LevelRendererSkyPassMixin` or the FrameGraph wiring; vanilla `addMainPass` runs after sky and rebinds depth as normal so terrain occlusion is unaffected. Both fabric + neoforge `./gradlew build` SUCCESSFUL. **In-game verify**: Earth from earth_orbit should fully occlude stars and the sun — no white pinpricks inside the disc, no bright square bleeding through. Backlight halo around the edge is correct. If two Ad Astra discs at the same `y=100` radius end up overlapping at the same screen pixel, the later-drawn one will overpaint via LEQUAL — easy follow-up is back-to-front sort by `scale` desc, but in practice angular separation makes this a non-issue.
